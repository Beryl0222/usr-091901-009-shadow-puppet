...
class ServiceContractTest(unittest.TestCase):
    ...
    def read_json(self, path):
        with urlopen(f"{self.base_url}{path}", timeout=2) as response:
            self.assertEqual(response.status, 200); self.assertEqual(response.headers.get_content_type(), "application/json")
    ...
    def test_contract_identity_and_rules(self):
        contract = self.read_json("/contract"); self.assertEqual(contract, load_contract())
        self.assertEqual(contract["service_id"], SERVICE_ID); self.assertGreaterEqual(len(contract["invariants"]), 3)
